...
export const Route = createFileRoute("/products/$id")({
  head: ({ params }) => ({
    meta: [
      { title: `منتج ${params.id} | LOMA` },
      { name: "description", content: "تفاصيل منتج لوما للعناية بالبشرة." },
...
function ProductDetail() {
  ...
  const { products, getProduct } = useSiteStore();
  ...
  useEffect(() => {
    if (!product) return;
    setVariantId(product.variants?.[0]?.id ?? product.id);
      ...
      <SiteShell>
        <section className="mx-auto grid min-h-[60svh] max-w-[1320px] place-items-center px-5 text-center">
          <div>
            ...
            <Button asChild variant="luxury" size="luxury" className="mt-8">
              <Link to="/products">عودة إلى المجموعة</Link>
            </Button>
          </div>
        </section>
      </SiteShell>
  ...
  const addToCart = () => {
    changeQuantity(variantId || product.id, quantity);
    setAdded(true);
      ...
      <section className="px-5 pb-16 pt-8 md:px-10 md:pb-24 md:pt-12 lg:px-14">
        <div className="mx-auto max-w-[1320px]">
          ...
          <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
            <div>
              <div
                className="relative aspect-[4/5] touch-pan-y overflow-hidden bg-secondary"
                onTouchStart={(event) => {
                ...
                {gallery.length > 1 && (
                  <>
                    <Button
                      ...
                      className="absolute right-4 top-1/2 size-10 -translate-y-1/2 rounded-full border border-border/70 bg-background/80 shadow-sm backdrop-blur-sm"
                    ...
                    <Button
                      ...
                      className="absolute left-4 top-1/2 size-10 -translate-y-1/2 rounded-full border border-border/70 bg-background/80 shadow-sm backdrop-blur-sm"
                  ...
                  {gallery.map((image, index) => (
                    <button
                      ...
                      className={`size-20 overflow-hidden border ${
                        activeImageIndex === index ? "border-foreground" : "border-border"
                      }`}
            ...
            <div className="lg:pt-4">
              ...
              <div className="mt-8 flex flex-wrap items-center gap-4">
                ...
                <Button variant="luxury" size="luxury" className="min-w-[220px] flex-1" onClick={addToCart}>
                  {added ? (
                    <>
                      <Check className="ml-2 size-4" /> أُضيف إلى السلة
                    </>
                  ) : (
                    `أضيفي إلى السلة — ${formatPrice(price * quantity)}`
                  )}
                </Button>
              </div>
